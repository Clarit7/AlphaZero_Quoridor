...
class Quoridor(object):
    ...
    def reset(self, first_player=1, second_player=2):
        ...
        self.dist = {1: BOARD_SIZE - 1, 2: BOARD_SIZE - 1}

        self.states = deque(maxlen=HISTORY_LEN * 2)
        self.widestates = deque(maxlen=HISTORY_LEN * 2)

    def additional_info(self):
        return np.array([self._player_walls_remaining[self.current_player], self._player_walls_remaining[3 - self.current_player], self.dist[self.current_player], self.dist[3 - self.current_player] ])


    def widestate2(self, flipped=False):
        """
        ...
        pos = np.zeros([BOARD_SIZE * 2 - 1, BOARD_SIZE * 2 - 1])
        ...
        pos[player1_row, player1_col] = 1
        pos[player2_row, player2_col] = -1
    ...
    def state(self):
        ...
        if self.current_player == 1:
            dist1_plane = np.full((1, BOARD_SIZE, BOARD_SIZE), self.dist[1])
            dist2_plane = np.full((1, BOARD_SIZE, BOARD_SIZE), self.dist[2])
        else:
            dist1_plane = np.full((1, BOARD_SIZE, BOARD_SIZE), self.dist[2])
            dist2_plane = np.full((1, BOARD_SIZE, BOARD_SIZE), self.dist[1])
    ...
    def step(self, action):
        ...
        self.dist[1], self.dist[2] = self.get_shortest_path()

        new_dis = self.dist[player]

        if new_dis >= cur_dis and action < 12:
            self._player_waste_move[player] -= 1


        if self.dist[1] == -1 or self.dist[2] == -1:
    ...
    def has_a_winner(self):
        ...
        if self._positions[2] > (BOARD_SIZE - 1) * BOARD_SIZE - 1:
            ...
            return True, 1
        if self.dist[1] - self.dist[2] > 1 and self._player_walls_remaining[1] == 0:
            return True, 2
        elif self.dist[2] - self.dist[1] > 1 and self._player_walls_remaining[2] == 0:
            return True, 1
    ...
    def improved_blocks_path(self, wall_location, orientation):
        player1_target = 0
        player2_target = BOARD_SIZE - 1
        ...
        adjacency_matrix_clone = self.adjacency_matrix.copy()


        column = wall_location % (BOARD_SIZE-1)
        row = wall_location // (BOARD_SIZE -1)

        if(orientation == HORIZONTAL):
            adjacency_matrix_clone[row * BOARD_SIZE + column][ (row+1) * BOARD_SIZE + column] = 0
            adjacency_matrix_clone[(row+1) * BOARD_SIZE + column][ row * BOARD_SIZE + column] = 0

            adjacency_matrix_clone[row * BOARD_SIZE + column + 1][ (row+1) * BOARD_SIZE + column + 1] = 0
            adjacency_matrix_clone[(row+1) * BOARD_SIZE + column + 1][ row * BOARD_SIZE + column + 1] = 0

        elif(orientation == VERTICAL):
            adjacency_matrix_clone[row * BOARD_SIZE + column][ row * BOARD_SIZE + column + 1] = 0
            adjacency_matrix_clone[row * BOARD_SIZE + column + 1][ row * BOARD_SIZE + column] = 0

            adjacency_matrix_clone[(row+1) * BOARD_SIZE + column ][ (row+1) * BOARD_SIZE + column + 1] = 0
            adjacency_matrix_clone[(row+1) * BOARD_SIZE + column + 1][ (row+1) * BOARD_SIZE + column] = 0


        G = self.to_graph_tool(adjacency_matrix_clone)

        shortest_path  = shortest_distance(G)

        player1_valid = min([shortest_path[player1_position][i] for i in range(BOARD_SIZE)])
        player2_valid = min([shortest_path[player2_position][i] for i in range(BOARD_SIZE*(BOARD_SIZE-1), BOARD_SIZE**2)])

        player1_valid = not (player1_valid > BOARD_SIZE ** 2)
        player2_valid = not (player2_valid > BOARD_SIZE ** 2)
    ...
    def start_test_play(self, alpha_player, pure_player, is_shown=0, temp=1e-3, first=1):
        ...
        while (1):
            ...
            tic = time.time()
            if self.current_player == 1:
                move, move_probs, q_vals = alpha_player.choose_action(self, return_prob=True)
                print("==============================================")
                print("\nturn %s" % time_step)
                # print('[Move probs]\n', move_probs[:12])
                # print('[Wall probs]\n', move_probs[12:])
                # print("alpha_player chose move : %s, prob: %.3f, q_vals: %.3f" % (move, move_probs[move], q_vals[move]))
                print("AlphaZero_player chose move : {}".format(move))
            else:
                move = pure_player.choose_action(self)
                print("\n==============================================")
                print("\nturn %s" % time_step)
                print("Minimax_player chose move : {}".format(move))
            ...
            print("Player 1 Shortest Path: {} Remaining Walls: {}, Player 2 Shortest Path: {} Remaining Walls: {}".format(self.dist[1], self._player_walls_remaining[1],  self.dist[2], self._player_walls_remaining[2] ))
