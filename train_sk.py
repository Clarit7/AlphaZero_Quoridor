...
class TrainPipeline(object):
    ...
    def get_simple_equi_data(self, play_data):
        ...
        for i, (state, mcts_prob, winner) in enumerate(play_data):
            ...
            h_equi_state.append(np.fliplr(state[0, :, :]))
            h_equi_state.append(np.fliplr(state[1, :, :]))
            v_equi_state.append(np.flipud(state[0, :, :]))
            v_equi_state.append(np.flipud(state[1, :, :]))

            h_equi_state = np.array(h_equi_state)
            v_equi_state = np.array(v_equi_state)

            hv_equi_state.append(np.flipud(h_equi_state[0, :, :]))
            hv_equi_state.append(np.flipud(h_equi_state[1, :, :]))

            hv_equi_state = np.array(hv_equi_state)
    ...
    def get_wide_equi_data(self, play_data):
        ...
        for i, (state, mcts_prob, winner) in enumerate(play_data):
            ...
            for i in range(2):
                ...
                flipped_player_position.append(np.flipud(player_position[i]))
            ...
            v_equi_state = np.vstack([flipped_wall_state, flipped_player_position, state[5:7, :, :], cur_player, state[8:, :, :]])
            ...
            for i in range(2):
                ...
                flipped_player_position.append(np.fliplr(np.flipud(player_position[i])))
            ...
            hv_equi_state = np.vstack([flipped_wall_state, flipped_player_position, state[5:7, :, :], cur_player, state[8:, :, :]])
    ...
    def collect_selfplay_data(self, n_games=1):
        for i in range(n_games):
            ...
            play_data = self.get_simple_equi_data(play_data)
    ...
    def run(self):
        try:
            ...
            writer.add_scalar("Win Ratio against minimax player", win_ratio, 0)
            ...
            for i in range(self.game_batch_num):
                ...
                if (i + 1) % self.check_freq == 0:
                    ...
                    writer.add_scalar("Win Ratio against minimax player", win_ratio, i)
